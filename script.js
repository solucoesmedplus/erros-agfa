...
function cacheElements() {
  const ids = [
    ...
    "newErrorFields", "closeNewErrorBtn", "cancelNewErrorBtn", "saveNewErrorBtn",
    "newErrorEyebrow", "newErrorTitle", "newErrorDescription", "saveNewErrorLabel",
    "deleteErrorModal", "deleteErrorCode", "deleteErrorEquipment", "cancelDeleteErrorBtn",
    "confirmDeleteErrorBtn", "toast"
  ];
...
function bindEvents() {
  ...
  elements.cancelDeleteErrorBtn.addEventListener("click", closeDeleteErrorModal);
  elements.confirmDeleteErrorBtn.addEventListener("click", confirmDeleteError);
  elements.deleteErrorModal.querySelector("[data-close-delete-error]").addEventListener("click", closeDeleteErrorModal);
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (!elements.deleteErrorModal.hidden) {
...
function openNewErrorModal() {
  ...
  elements.newErrorEquipment.disabled = false;
  elements.newErrorEquipment.value = equipmentKey;
  configureErrorFormUi();
  renderNewErrorFields();
  openErrorFormModal();
}

function openEditErrorModal(record) {
  if (!state.xmlDocument || !record || !state.selectedEquipmentKey) {
    showToast("Não foi possível localizar o erro para edição.");
    return;
  }
  ...
  state.editingRecord = {
    ...
    originalIdentifier: record.identifier
  };

  elements.newErrorForm.reset();
  elements.newErrorEquipment.value = state.selectedEquipmentKey;
  elements.newErrorEquipment.disabled = true;
  configureErrorFormUi();
  renderNewErrorFields();

  const fieldValues = Object.fromEntries(record.fields.map((field) => [field.name, field.translated]));
  fieldValues.codigo_erro = record.code;
  fieldValues.identificador = record.identifier;

  Object.entries(fieldValues).forEach(([fieldName, value]) => {
    const control = elements.newErrorFields.querySelector(`[name="${fieldName}"]`);
...
function configureErrorFormUi() {
  const isEdit = state.formMode === "edit";

  elements.newErrorEyebrow.textContent = isEdit ? "Edição manual" : "Cadastro manual";
  elements.newErrorTitle.textContent = isEdit ? "Editar Erro AGFA" : "+ Novo Erro AGFA";
  elements.newErrorDescription.textContent = isEdit
    ? "Atualize os dados técnicos do erro selecionado. O equipamento permanece fixo durante a edição."
    : "Preencha os dados técnicos. Os campos exibidos mudam conforme o equipamento selecionado.";
  elements.saveNewErrorLabel.textContent = isEdit ? "Salvar alterações e gerar XML" : "Adicionar e gerar XML";
}
...
function closeNewErrorModal() {
  ...
  if (
    elements.introModal.hidden &&
    elements.xmlErrorModal.hidden &&
    elements.deleteErrorModal.hidden &&
...
async function handleNewErrorSubmit(event) {
  ...
  const currentNode = state.formMode === "edit"
    ? findErrorNode(equipmentNode, state.editingRecord)
    : null;

  if (state.formMode === "edit" && !currentNode) {
    showToast("O erro original não foi encontrado no XML.");
    return;
  }

  const duplicate = Array.from(equipmentNode.querySelectorAll(":scope > erros > erro")).find((errorNode) => {
    if (errorNode === currentNode) return false;

    return cleanText(errorNode.getAttribute("codigo")) === code ||
      cleanText(errorNode.getAttribute("identificador")) === identifier;
  });
  ...
  try {
    if (state.formMode === "edit") {
      updateExistingError(currentNode, equipmentNode, equipmentKey, values);
      closeNewErrorModal();
      ...
      applySearch(code);
      downloadUpdatedXml();
      showToast(`Erro ${code} atualizado. O XML atualizado foi gerado.`);
    } else {
    ...
    console.error("Falha ao salvar erro:", error);
    showToast("Não foi possível salvar o erro. Revise os dados e tente novamente.");
  } finally {
...
function findErrorNode(equipmentNode, reference) {
  if (!equipmentNode || !reference) return null;

  const nodes = Array.from(equipmentNode.querySelectorAll(":scope > erros > erro"));
  const xmlIndex = String(reference.xmlIndex ?? "");

  return nodes.find((node) => xmlIndex && node.getAttribute("indice") === xmlIndex) ||
    nodes.find((node) => {
      return cleanText(node.getAttribute("codigo")) === cleanText(reference.originalCode || reference.code) &&
        cleanText(node.getAttribute("identificador")) === cleanText(reference.originalIdentifier || reference.identifier);
    }) ||
    null;
}

function updateExistingError(errorNode, equipmentNode, equipmentKey, values) {
  errorNode.setAttribute("codigo", values.codigo_erro);
  errorNode.setAttribute("identificador", values.identificador);
  errorNode.setAttribute("editado_manualmente", "sim");

  let fieldsNode = errorNode.querySelector(":scope > campos");
  if (!fieldsNode) {
    fieldsNode = state.xmlDocument.createElement("campos");
  ...
  fieldNames.forEach((fieldName) => {
    ...
    let fieldNode = Array.from(fieldsNode.querySelectorAll(":scope > campo"))
      .find((node) => node.getAttribute("nome") === fieldName);

    if (!fieldNode) {
      fieldNode = state.xmlDocument.createElement("campo");
    ...
    let originalNode = fieldNode.querySelector(":scope > original");
    if (!originalNode) {
      originalNode = state.xmlDocument.createElement("original");
      originalNode.setAttribute("idioma", "en");
      fieldNode.insertBefore(originalNode, fieldNode.firstChild);
    }

    let translatedNode = fieldNode.querySelector(":scope > traducao");
    if (!translatedNode) {
      translatedNode = state.xmlDocument.createElement("traducao");
      translatedNode.setAttribute("idioma", "pt-BR");
      fieldNode.appendChild(translatedNode);
    }

    const technicalPreserved = ["codigo_erro", "identificador", "id_servico", "codigo_servico"].includes(fieldName);
    if (technicalPreserved) {
      originalNode.textContent = value;
    }

    translatedNode.setAttribute("metodo", technicalPreserved ? "valor_tecnico_preservado" : "edicao_manual");
    translatedNode.textContent = value;
  });

  refreshXmlMetadata(equipmentNode);
}

function refreshXmlMetadata(equipmentNode) {
  if (equipmentNode) {
    const errors = Array.from(equipmentNode.querySelectorAll(":scope > erros > erro"));
    equipmentNode.setAttribute("total_erros", String(errors.length));

    errors.forEach((node, index) => {
      node.setAttribute("indice", String(index + 1));
    });
    ...
    summaryItem?.setAttribute("total_erros", String(errors.length));
  }
  ...
  const totalEquipments = Number(root.getAttribute("total_equipamentos")) ||
    state.xmlDocument.querySelectorAll("equipamento").length;

  setDatabaseStatus("ready", `${formatNumber(totalErrors)} erros • ${totalEquipments} equipamentos`);
}

function openDeleteErrorModal(record) {
  if (!record || !state.selectedEquipmentKey) {
    showToast("Não foi possível localizar o erro para exclusão.");
    return;
  }

  const equipment = EQUIPMENTS[state.selectedEquipmentKey];
  state.pendingDelete = {
    equipmentKey: state.selectedEquipmentKey,
    xmlIndex: record.xmlIndex,
    originalCode: record.code,
  ...
  elements.deleteErrorCode.textContent = record.code;
  elements.deleteErrorEquipment.textContent = equipment?.title || "equipamento selecionado";
  elements.deleteErrorModal.hidden = false;
  document.body.classList.add("is-locked");
  elements.confirmDeleteErrorBtn.focus({ preventScroll: true });
}

function closeDeleteErrorModal() {
  elements.deleteErrorModal.hidden = true;
  state.pendingDelete = null;

  if (
    elements.introModal.hidden &&
    elements.xmlErrorModal.hidden &&
    elements.newErrorModal.hidden &&
    !elements.sidebar.classList.contains("is-open")
  ) {
    document.body.classList.remove("is-locked");
  }
...
async function confirmDeleteError() {
  if (!state.pendingDelete || !state.xmlDocument) {
    closeDeleteErrorModal();
    return;
  }

  const reference = { ...state.pendingDelete };
  const equipment = EQUIPMENTS[reference.equipmentKey];
  const equipmentNode = findEquipmentNode(equipment?.xmlName);

  if (!equipmentNode) {
    showToast("O equipamento não foi encontrado no XML.");
    closeDeleteErrorModal();
    return;
  }

  const errorNode = findErrorNode(equipmentNode, reference);
  if (!errorNode) {
    showToast("O erro selecionado não foi encontrado no XML.");
    closeDeleteErrorModal();
    return;
  }

  const deletedCode = cleanText(errorNode.getAttribute("codigo")) || reference.originalCode;
  elements.confirmDeleteErrorBtn.disabled = true;

  try {
    removeErrorNode(errorNode);
    refreshXmlMetadata(equipmentNode);
    closeDeleteErrorModal();

    await selectEquipment(reference.equipmentKey, { preserveMenu: true });
    elements.searchInput.value = "";
    elements.clearSearchBtn.hidden = true;
    applySearch("");

    downloadUpdatedXml();
    showToast(`Erro ${deletedCode} excluído. O XML atualizado foi gerado.`);
  } catch (error) {
    console.error("Falha ao excluir erro:", error);
    showToast("Não foi possível excluir o erro.");
  } finally {
    elements.confirmDeleteErrorBtn.disabled = false;
  }
}

function removeErrorNode(errorNode) {
  const previousSibling = errorNode.previousSibling;
  errorNode.remove();
...
function closeSidebar() {
  ...
  if (elements.introModal.hidden && elements.xmlErrorModal.hidden && elements.newErrorModal.hidden && elements.deleteErrorModal.hidden) {
    document.body.classList.remove("is-locked");
  }
}
...
function buildEquipmentRecords(xmlEquipmentName) {
  ...
  return Array.from(equipmentNode.querySelectorAll(":scope > erros > erro")).map((errorNode, index) => {
    ...
    return {
      index,
      xmlIndex: errorNode.getAttribute("indice") || String(index + 1),
      code,
      identifier,
      priority,
      description,
  ...
  return `
    <article class="error-card" data-code="${escapeAttribute(record.code)}">
      <div class="error-card__top">
        ...
        <div class="error-card__actions" aria-label="Ações do erro">
          <button class="card-action-button copy-button" type="button" data-copy-code="${escapeAttribute(record.code)}" aria-label="Copiar código ${escapeAttribute(record.code)}" title="Copiar código">
          ...
          <button class="card-action-button edit-error-button" type="button" data-edit-error="${escapeAttribute(record.xmlIndex)}" aria-label="Editar erro ${escapeAttribute(record.code)}" title="Editar erro">
            <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L8 18l-4 1 1-4Z"/></svg>
          </button>
          <button class="card-action-button delete-error-button" type="button" data-delete-error="${escapeAttribute(record.xmlIndex)}" aria-label="Excluir erro ${escapeAttribute(record.code)}" title="Excluir erro">
            <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 6h18M8 6V4h8v2m-9 0 1 14h8l1-14M10 10v6M14 10v6"/></svg>
          </button>
        </div>
      </div>
...
function handleResultsClick(event) {
  const editButton = event.target.closest("[data-edit-error]");
  if (editButton) {
    const record = findRecordByXmlIndex(editButton.dataset.editError);
    openEditErrorModal(record);
    return;
  }

  const deleteButton = event.target.closest("[data-delete-error]");
  if (deleteButton) {
    const record = findRecordByXmlIndex(deleteButton.dataset.deleteError);
    openDeleteErrorModal(record);
    return;
  }
...
function findRecordByXmlIndex(xmlIndex) {
  return state.records.find((record) => String(record.xmlIndex) === String(xmlIndex)) || null;
}
